...
Device.prototype._popAndSend = function (deviceId) {
	var registrationObject = this._registeredDevices [deviceId];
    if (registrationObject.pending.length) this.sendPush (registrationObject, registrationObject.pending[0]);
};

Device.prototype.pushSuccessful = function (deviceId) {
	var registrationObject = this._registeredDevices[deviceId];
    this.emit ("push-success", deviceId, registrationObject.pending[0].pushId);
    registrationObject.pending.splice (0, 1);
    registrationObject.lastInterval = 1;
    if (registrationObject.pending.length) this._popAndSend ();
};
...
Device.prototype.pushFailed = function (deviceId, retryAfter /* in seconds */) {
    ...
    if (retryAfter) {
        ...
        interval = registrationObject.lastInterval;
        registrationObject.lastInterval *= 2;
    }
    registrationObject.timer = setTimeout (this._popAndSend.bind (this), interval * 1000);
};
